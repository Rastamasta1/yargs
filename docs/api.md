...
`include`: Allow list certain modules. Either a regex or callback can be provided. Return `true` from the callback to load the file.
...
Later on, `argv` can be retrieved with `y.parse()`.
